...
import com.kuro.mdp.charts.bars.config.StackBarConfig
import com.kuro.mdp.charts.bars.model.StackBarData
...
private fun LineStackBarChartContent(
    ...
    val lineData = data()
    val displayData =
        remember(lineData) { getDisplayData(lineData, stackBarConfig.minimumBarCount) }
    val maxValue =
        remember(displayData) { displayData.maxOfOrNull { it.values.sum().absoluteValue } ?: 0f }
    val hasNegativeValues =
        remember(lineData) { displayData.fastFlatMap { it.values }.fastAny { it < 0 } }
