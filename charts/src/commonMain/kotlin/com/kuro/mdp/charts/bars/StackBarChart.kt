...
private fun StackBarChartContent(
    ...
    val displayData =
        remember(stackBarData) { getDisplayData(stackBarData, stackBarConfig.minimumBarCount) }
    val maxValue =
        remember(displayData) {
            displayData.fastMaxOfOrNull { it.values.sum().absoluteValue } ?: 0f
        }
    val hasNegativeValues =
        remember(stackBarData) { displayData.fastFlatMap { it.values }.fastAny { it < 0 } }
